...
import time, random, sys, json, codecs, threading, glob, re, string, os, requests, subprocess, six, ast, pytz, urllib.request, urllib.parse, urllib.error, urllib. parse, subprocess, unicodedata, GACSender
...
#line = LINE()
#line = LINE("เมล","พาส")
line = LINE('Evc4Ld1HSmjCx8FAHi9a.ldOTsESu/yPLNuj3RhICoG.DjXOWduckIpeX5K7QxsGwhqLWAVdLmw/45+s/NyBR3M=')
...
RfuBot=[lineMID]
Family=["u2c3a2d49eb14f98a659d4277ac789c3a",lineMID]
admin=['u2c3a2d49eb14f98a659d4277ac789c3a',lineMID]
...
settings = {
    ...
    "kick":"😱อุ๊ต๊ะ..คือโหดแท้ว่ะ😱",
    "bye":"🙌บ๊ายบาย..แล้วพบกันใหม่นะ😖",
    "Respontag":"😳",
    ...
    "comment":"""สนใจอยากได้ไปเล่นทักได้นะ อิอิ """,
    "userAgent": [
...
dangerMessage2 = ["ควย","พ่องตาย","ควยไร","สัส","ควยไร","ไอ้สัส","กะหรี่","อิดอก","ไอ้เวน","ไอ้เห้ย","ไอ้ควาย","ไอ้หน้าหี"]
...
#==============================================================================#
                    ...
                    line.sendMessage(to, "เปิดระบบออโต้บล็อค.")
                elif text.lower() == '.ปิดบล็อค':
                    settings["autoAdd"] = False
                    line.sendMessage(to, "ปิดระบบออโต้บล็อค.")
                elif text.lower() == '.เปิดเข้า':
                    settings["autoJoin"] = True
                    line.sendMessage(to, "เปิดระบบเข้ากลุ่มอัตโนมัติ.")
                elif text.lower() == '.ปิดเข้า':
                    settings["autoJoin"] = False
                    line.sendMessage(to, "ปิดระบบเข้ากลุ่มอัตโนมัติ.")
                    ...
                    line.sendMessage(to, "เปิดระบบเช็คติ๊กอัตโนมัติ.")
                elif text.lower() == '.ปิดติ๊ก':
                    settings["checkSticker"] = False
                    line.sendMessage(to, "ปิดระบบเช็คติ๊กอัตโนมัติ.")
                elif text.lower() == '.เปิดมุด':
                    settings["autoJoinTicket"] = True
                    line.sendMessage(to, "เปิดระบบมุดลิ้งอัตโนมัติ.")
                elif text.lower() == '.ปิดมุด':
                    settings["autoJoinTicket"] = False
                    line.sendMessage(to, "ปิดระบบมุดลิ้งอัตโนมัติ.")
                elif text.lower() == '.เปิดเผือก':
                    settings["unsendMessage"] = True
                    line.sendMessage(to, "เปิดระบบดูยกเลิกแชทอัตโนมัติ.")
                elif text.lower() == '.ปิดเผือก':
                    settings["unsendMessage"] = False
                    line.sendMessage(to, "ปิดระบบดูยกเลิกแชทอัตโนมัติ.")           
#==============================================================================#
                  ...
                  if msg._from in admin:
                    ...
                    try:
                        ...
                        line.sendMessage(msg.to, "〘•คืนร่างเดิมแล้ว•〙")
                    except Exception as e:
                        line.sendMessage(msg.to, str (e))
                        ...
                        line.sendMessage(msg.to,"การตั้งค่าชุดรักษาความปลอดภัยทั้งหมด เปิด👌")
                        ...
                        line.sendMessage(msg.to,"การตั้งค่าชุดรักษาความปลอดภัยทั้งหมด ปิด👌")
                        ...
                        settings["Api"] = True
                        line.sendMessage(msg.to,"การตั้งค่าชุดข้อความทั้งหมด เปิด👌")
                        ...
                        settings["Api"] = False
                        line.sendMessage(msg.to,"การตั้งค่าชุดข้อความทั้งหมด ปิด👌")
#==============================================================================#
...
#==============FINNISHING PROTECT========================#
                      ...
                      if msg.toType == 2:
                       ...
                       for target in targets:
                           try:
                               ...
                               f=codecs.open('st2__b.json','w','utf-8')
                               json.dump(settings["blacklist"], f, sort_keys=True, indent=4,ensure_ascii=False)
                               line.sendMessage(msg.to,"เพิ่มบัญชีสำเร็จ")
                               print ("Banned User")
                           except:
                               line.sendMessage(msg.to,"Contact Not Found")
                       ...
                       for target in targets:
                           try:
                               ...
                               f=codecs.open('st2__b.json','w','utf-8')
                               json.dump(settings["blacklist"], f, sort_keys=True, indent=4,ensure_ascii=False)
                               line.sendMessage(msg.to,"ยกเลิกบัญชีดำสำเร็จ. ")
                               print ("Unbanned User")
                           except:
                               line.sendMessage(msg.to,"Contact Not Found")
                  ...
                  if msg._from in Family:
                    if settings["blacklist"] == {}:
                        line.sendMessage(msg.to,"ไม่พบรายชื่อผู้ติดดำ") 
                    else:
...
#==============================================================================#
        ...
        if op.type == 26:
            ...
            if settings ["Aip"] == True:
                if msg.text in dangerMessage:
                    random.choice(Rfu).kickoutFromGroup(receiver,[sender])
                    random.choice(Rfu).sendMessage(msg.to,"ตรวจพบคำพูดหยาบคายไม่สุภาพ จำเป็นต้องนำออกเพื่อความสงบสุขของสมาชิก (｀・ω・´)")
            if settings ["Api"] == True:
            	if msg.text in ["ป๊า","ป๊าวี","ลุงวี","wee","พี่วี","วี","ทวดวี","เฮีย"]:
                    line.sendMessage(msg.to, "เรียกทำไมเหงาหรอ😒😒")
            if settings ["Api"] == True:
            ...
            if settings ["Api"] == True:
                if msg.text in ["55","555","5555","55555","55+","555+","5555+","ขำ",".ขำ"]:
                    line.sendMessage(msg.to,"ฮ่าๆๆๆ..ขำไร..ขำด้วยคนดิ")
            if settings ["Api"] == True:
