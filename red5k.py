...
#==============================================================================#
                    ...
                    if msg.from_ in admin:
                        ...
                        if gid == "":
                            line.sendMessage(msg.to,"โปรดส่งไอดีกลุ่ม")
                        else:
                            try:
                                ...
                                line.sendMessage(msg.to,"เชิญเข้ากลุ่มที่สั่งเรียบร้อยแล้ว")
                  ...
                  if msg._from in admin:
                    ...
                    try:
                        ...
                        settings["Ghost"] = True
                        line.sendMessage(msg.to,"เปิดระบบเรียกคิกผีแล้วท่าน..(○ﾟεﾟ○)")

                elif msg.text in ["Ghost on",".ปิดคิกผี"]:
                        settings["Ghost"] = False
                        line.sendMessage(msg.to,"ปิดระบบเรียกคิกผีเรียบร้อย..(´⊙ω⊙`)")
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                               ...
                               ki.sendMessage(msg.to,"🏂⛷️[จะออกไปแตะขอบฟ้า]")
                               ke.createGroup("RED SAMURI Group",mi_d)
                               ke.sendMessage(msg.to,"🏂⛷️[จะออกไปแตะขอบฟ้า]")
                               kk.createGroup("RED SAMURI Group",mi_d)
                               kk.sendMessage(msg.to,"🏂⛷️[จะออกไปแตะขอบฟ้า]")
                               kcc.createGroup("RED SAMURI Group",mi_d)
                               kc.sendMessage(msg.to,"🏂⛷️[จะออกไปแตะขอบฟ้า]")
                               line.createGroup("RED SAMURI Group",mi_d)
                               line.sendMessage(msg.to,"🏂⛷️[จะออกไปแตะขอบฟ้า]")
                               ki.createGroup("RED SAMURI Group",mi_d)
                               ki.sendMessage(msg.to,"🏂⛷️[จะออกไปแตะขอบฟ้า]")
        ...
        if op.type == 13:
                if op.param2 not in Family:
                    if op.param2 in Family:
                        ...
                        line.sendMessage(op.param1,"™❍✯͜͡REDSAMURAI✯͜͡❂➣ ") 
                    if op.param2 not in Family:
                        if op.param2 in Family:
                            pass
                    elif RfuProtect["cancelprotect"] == True:
                        ...
                        line.sendMessage(op.param1,"❇️ มีการเชิญสมาชิกเข้าร่วมกลุ่ม ❇️ ")

        if op.type == 17:
           if op.param2 in settings["blacklist"]:
               if op.param2 not in Family:
                random.choice(Rfu).kickoutFromGroup(op.param1,[op.param2])
                line.sendMessage(op.param1,"สมาชิกที่ถูกแบนไม่ได้รับอนุญาตให้เข้าร่วมกลุ่ม （´・ω・｀）")
        if op.type == 17:
           if RfuProtect["Protectjoin"] == True:
               if op.param2 not in Family:
                   random.choice(Rfu).kickoutFromGroup(op.param1,[op.param2])
